...
CProcessViewDlg::CProcessViewDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CProcessViewDlg::IDD, pParent),m_ppt(_T(""))
{
	m_StatusBar=NULL;
	m_bRunning=false;
...
// 	SetMenu(&m_CMenuMain);
	...
	m_ppt.Create(this,WS_OVERLAPPEDWINDOW|WS_VISIBLE);
...
void CProcessViewDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CProcessViewDlg::OnLvnItemchangedList1(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMLISTVIEW pNMLV = reinterpret_cast<LPNMLISTVIEW>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	*pResult = 0;
}
void CProcessViewDlg::OnSize(UINT nType, int cx, int cy) 
{
...
void CProcessViewDlg::OnFileExit()
{
	// TODO: 在此添加命令处理程序代码
	this->OnCancel();
}
void CProcessViewDlg::InitProcessList(){
	PPROCESSNODE p;
...
void CProcessViewDlg::OnTimer(UINT nIDEvent){
	if(nIDEvent==1)
		InitProcessList();
	else if(nIDEvent==2){
...
void CProcessViewDlg::OnFileStartrefresh()
{
	if(m_bRunning){
		KillTimer(1);
		m_bRunning=false;
...
void CProcessViewDlg::OnUpdateFileStartrefresh(CCmdUI *pCmdUI)
{
	if(m_bRunning)
		pCmdUI->SetCheck(0);
	else
